...
const TransactionPage = () => {
  ...
  const viewRow = (row) => {
    setSelectedItem(row);
    setOpenTransactionModal(row);
  };

  const onTableItemClick = (row) => {
    setSelectedItem(row.original);
    setOpenTransactionModal(row.original.orderID);
  };
    ...
    <div className="flex w-full gap-4 flex-col">
      ...
      >
        <DialogContent className="flex flex-col p-6  gap-6 w-full sm:max-w-[426px] max-h-[calc(100dvh-50px)]  overflow-y-auto">
          ...
          <Button
            onClick={() =>
              navigate(SUPER_ADMIN_ROUTES.eachOrder + selectedItem?.orderID)
            }
      ...
      <section className="flex flex-col py-7 px-5 md:px-6 gap-4 bg-white rounded-[8px] w-full">
        <div className="flex flex-col md:flex-row items-start gap-[19px] w-full">
          ...
          />
          <TransactionCard heading={'Total groceries sales'} subtitle={'0'} />
          <TransactionCard heading={'Total Utility bill'} subtitle={'0'} />
          <TransactionCard heading={'Total Donation'} subtitle={'0'} />
        </div>
